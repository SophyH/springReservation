...
public interface ClientRepository extends JpaRepository<Client, Long>{
	...
	@Query("select distinct cei from ClientEI cei")
	public List<ClientEI> findAllCustomClientEI();
	
	@Query("select distinct cm from ClientMoral cm")
	public List<ClientMoral> findAllCustomClientMoral();
	
	@Query("select distinct cp from ClientPhysique cp")
	public List<ClientPhysique> findAllCustomClientPhysique();
	
	@Query("select distinct c from Client c left join fetch c.reservations where c.login = :login")
